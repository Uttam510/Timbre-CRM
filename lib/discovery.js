...
export async function processQuery({ supabase, query, segment, minSubs = 5000, maxSubs = 200000, emit = () => {}, deadline = 0, maxScrape = Infinity }) {
  let saved = 0, found = 0, scrapes = 0;
  const timeLeft = () => !deadline || Date.now() < deadline;
  ...
  for (const ch of details.items || []) {
    ...
    // Scraping is the slow part — skip it once we're out of time or over the cap
    // (the lead is still saved with its other details, just without a scraped email).
    if (!contactEmail && website && scrapes < maxScrape && timeLeft()) {
      scrapes++;
